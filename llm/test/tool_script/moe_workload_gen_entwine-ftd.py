...
def init_vars(input_vars):
    ...
    # Expert distribution: E_N/dp (each FTD member holds E_N/dp complete experts)
    if dp > 1:
        input_vars[f'E_N/{dp}'] = input_vars['experts'] // dp
...
def process_core_worklist_moentwine(input_vars, core_index, tp, core_layer, dp_index, base_id, total_cores, loop_count=1, phase="both"):
    ...
    btck_dp = f"BTCK/{dp}" if dp > 1 else "BTC"  # All-to-All data size per FTD peer
    # Local expert count: E_N/dp (not E_N/(dp*tp))
    en_local = input_vars['experts'] // dp if dp > 1 else input_vars['experts']
    if dp > 1:
        input_vars[f'E_N/{dp}'] = en_local
    en_dp_str = f"E_N/{dp}" if dp > 1 else "E_N"
...
def main():
    ...
    tp_parts = input_vars['tp'].split("_")
    tp = int(tp_parts[0]) * int(tp_parts[1])
    if dp > 1 and input_vars['experts'] % dp != 0:
        print(f"Error: experts ({input_vars['experts']}) must be divisible by dp ({dp}) in MoEntwine mode")
        return
    if dp <= 1:
